...
//*******************
Aria.classDefinition({$classpath:"aria.embed.Element",$extends:"aria.widgetLibs.BaseWidget",$dependencies:["aria.embed.CfgBeans","aria.utils.Html","aria.core.JsonValidator","aria.core.Log","aria.utils.Dom"],$constructor:function(t){this.$BaseWidget.constructor.apply(this,arguments),this._cfgOk=aria.core.JsonValidator.validateCfg(this._cfgBeanName,t)},$destructor:function(){this._domId&&(this._cfg.controller.onEmbededElementDispose?(this.$logWarn("onEmbededElementDispose has been deprecated, please use onEmbeddedElementDispose"),this._cfg.controller.onEmbededElementDispose(aria.utils.Dom.getElementById(this._domId),this._cfg.args)):this._cfg.controller.onEmbeddedElementDispose(aria.utils.Dom.getElementById(this._domId),this._cfg.args)),this.$BaseWidget.$destructor.apply(this,arguments)},$prototype:{_cfgBeanName:"aria.embed.CfgBeans.ElementCfg",writeMarkup:function(t){if(this._cfgOk){this._domId=this._createDynamicId();var e=this._cfg.type,i=["<",e,' id="',this._domId,'"'];this._cfg.attributes&&i.push(" "+aria.utils.Html.buildAttributeList(this._cfg.attributes)),i.push("></"+e+">"),t.write(i.join(""))}},initWidget:function(){this._cfgOk&&(this._cfg.controller.onEmbededElementCreate?(this.$logWarn("onEmbededElementCreate has been deprecated, please use onEmbeddedElementCreate"),this._cfg.controller.onEmbededElementCreate(aria.utils.Dom.getElementById(this._domId),this._cfg.args)):this._cfg.controller.onEmbeddedElementCreate(aria.utils.Dom.getElementById(this._domId),this._cfg.args))}}});
//*******************
...
//*******************
Aria.interfaceDefinition({$classpath:"aria.embed.IEmbedController",$interface:{onEmbeddedElementCreate:function(){},onEmbeddedElementDispose:function(){}}});
//*******************
